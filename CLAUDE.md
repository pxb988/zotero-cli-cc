...
- `commands/index_cmd.py` — `zot index build/status` for full-library semantic search. Builds a RagIndex at `~/.config/zot/index/<library_id>.idx.sqlite` over all library items, excluding trashed items (metadata chunks for every item + PDF chunks for items with attachments). The trash exclusion lives in `reader.get_all_item_ids()` (filters `deletedItems`), not in `index_cmd.py` itself. The `search --semantic` flag in `commands/search.py` loads this index for BM25 + embedding hybrid retrieval, aggregating chunk scores to item level.

...
def test_attachment_path_human_outputs_bare_existing_path(tmp_path: Path, test_db_path: Path):
    # Single-PDF item: the default (list all) still prints one bare path, pipe-friendly.
...
def test_attachment_path_default_json_returns_array(tmp_path: Path, test_db_path: Path):
    # Default JSON envelope is the array shape even for a single PDF.
    _copy_fixture_db(tmp_path, test_db_path)
    pdf_path = _create_pdf(tmp_path)
    ...
    att = env["data"]["attachments"][0]
    assert att["attachment_key"] == "ATCH005"
    assert att["path"] == str(pdf_path)
    assert att["filename"] == "attention.pdf"
    assert att["exists"] is True
    assert att["mime_type"] == "application/pdf"


def test_attachment_path_first_json_returns_single_object(tmp_path: Path, test_db_path: Path):
    # --first restores the pre-0.10 single-object envelope.
    _copy_fixture_db(tmp_path, test_db_path)
    pdf_path = _create_pdf(tmp_path)

    result = _invoke(["--json", "attachment", "path", "ATTN001", "--first"], tmp_path)
...
def test_attachment_path_default_lists_every_pdf(tmp_path: Path, test_db_path: Path):
    # Default (no flag) lists all PDFs, one bare path per line.
    _copy_fixture_db(tmp_path, test_db_path)
    translated_path = _create_pdf(tmp_path, "ATCH012", "translated_cn.pdf")
    original_path = _create_pdf(tmp_path, "ATCH013", "original.pdf")

    result = _invoke(["attachment", "path", "BILI011"], tmp_path)
...
def test_attachment_path_first_missing_local_file_is_not_found(tmp_path: Path, test_db_path: Path):
    _copy_fixture_db(tmp_path, test_db_path)
...
def test_attachment_path_default_missing_local_file_is_not_found(tmp_path: Path, test_db_path: Path):
    # Default lists all; with no local file it reports the all-style message.
    _copy_fixture_db(tmp_path, test_db_path)
...
def test_attachment_path_first_picks_first_pdf_for_multi_pdf_item(tmp_path: Path, test_db_path: Path):
    _copy_fixture_db(tmp_path, test_db_path)
    translated_path = _create_pdf(tmp_path, "ATCH012", "translated_cn.pdf")
    _create_pdf(tmp_path, "ATCH013", "original.pdf")

    result = _invoke(["--json", "attachment", "path", "BILI011", "--first"], tmp_path)

...
- `rag.py` + `rag_index.py` — BM25 index over workspace metadata + PDF text, with optional embedding-based hybrid retrieval. The embedding layer is provider-routed (`core/embedding_router.py`) — choose via `[embedding] provider` (jina/aliyun/zhipu) plus `ZOT_EMBEDDING_URL` / `ZOT_EMBEDDING_KEY`. Also powers the full-library semantic index (see `commands/index_cmd.py` below).
- `commands/index_cmd.py` — `zot index build/status` for full-library semantic search. Builds a RagIndex at `~/.config/zot/index/<library_id>.idx.sqlite` over all library items (metadata chunks for every item + PDF chunks for items with attachments). The `search --semantic` flag in `commands/search.py` loads this index for BM25 + embedding hybrid retrieval, aggregating chunk scores to item level.
...
- `skill/zotero-cli-cc/` is the Codex skill packaged with this repo; users install it by copying to `~/.Codex/skills/`. Keep it in sync with CLI surface changes.
